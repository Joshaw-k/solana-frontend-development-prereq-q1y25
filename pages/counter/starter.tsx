...
const Starter = () => {
    ...
    const provider = new AnchorProvider(connection, wallet?.adapter as unknown as NodeWallet, AnchorProvider.defaultOptions())

    const counterProgram = new Program(CounterIDL as unknown as Counter, provider)
    ...
    const handleInitializeCounter = async () => {
        ...
        try {
            const signature = await provider.sendAndConfirm(transaction, [counterKeypair], {
                skipPreflight: true
            })
            setTxSig(signature)
    ...
    const handleSetCounter = async () => {
        ...
        const instruction = await counterProgram.methods.set(3).accounts({
            counter: new PublicKey(counterKey)
        }).instruction()
    ...
    useEffect(() => {
        const getInfo = async () => {
            if (connection && publicKey && counterKey) {
                try {
                    const counterCurrent = await counterProgram.account.counter.fetch(new PublicKey(counterKey))
                    setCount(counterCurrent.count)
                } catch (error) {
                    console.log(error);
                }
            }
    ...
    return <main className="min-h-screen text-white max-w-7xl">
